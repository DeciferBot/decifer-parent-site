...
export function GET() {
  ...
  const lines: string[] = [
    ...
    "",
    ...stack.map((t) => `- ${t.name} (${t.category}): ${t.role} ${t.why}`),
    "",
    `Full reasoning, and how a job is routed between a deterministic path, an open-weight model and Claude: [The stack](${SITE}/stack).`,
    "",
    "## Learn",
    "",
